...
public class JavaApplicationModel extends ApplicationModel {
	...
	@Override
	public ApplicationModel addOnce(EntityModel<? extends Identifiable> entityModel) {
		...
		getEntityIds().stream().forEach(entityKey -> {
			for (String otherEntityKey : getEntityIds()) {
				if (!entityKey.equals(otherEntityKey)) {
					JavaEntityModel<? extends Identifiable> entityModel = (JavaEntityModel<? extends Identifiable>) getEntity(entityKey);
					JavaEntityModel<? extends Identifiable> otherEntityModel = (JavaEntityModel<? extends Identifiable>) getEntity(otherEntityKey);
