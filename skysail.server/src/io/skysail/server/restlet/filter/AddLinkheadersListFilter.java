...
public class AddLinkheadersListFilter extends AbstractListResourceFilter {
	...
	protected void afterHandle(SkysailServerResource<?> resource, Wrapper2 responseWrapper) {
		...
		String links = linkheaderAuthorized.stream().map(link -> link.toString("")).collect(Collectors.joining(","));
		...
		if (limitedLinks.length() < links.length()) {
			responseHeaders.add(new Header("X-Link-Error", "link header was too large: " + links.length()
					+ " bytes, cutting down to " + limitedLinks.length() + " bytes."));
		}
	...
	private String shrinkLinkHeaderSizeIfNecessary(int linkCount, String links) {
		...
		if (links.length() > MAX_LINK_HEADER_SIZE) {
			String reducedLinks = Arrays.stream(links.split(",", linkCount)).limit(linkCount - 1)
	 ...
	 */
	private Consumer<? super Link> getPathSubstitutions(SkysailServerResource<?> resource) {
		return l -> {
			String uri = l.getUri();
