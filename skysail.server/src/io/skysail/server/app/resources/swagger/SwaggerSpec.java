...
public class SwaggerSpec implements Entity {
	...
	public SwaggerSpec(SkysailApplication skysailApplication, Request request) {
		...
		this.host = determineHost(request);// request.getHostRef().getHostDomain() + ":" + request.getHostRef().getHostPort();
    ...
    private String determineHost(Request request) {
        String hostDomainAsIp = request.getHostRef().getHostDomain();// + ":" + request.getHostRef().getHostPort();
        try {
