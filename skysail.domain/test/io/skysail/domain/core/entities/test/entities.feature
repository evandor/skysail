...
Feature: entities specific features
  ...
  @Equality
  Scenario: two entites with the same name (i.e. id) are considered equal
    Given An Entity with name 'note'
	  And Another Entity with name 'note'
    Then the two entities are equal.

  @Equality
  Scenario: two entites with the same name (i.e. id) are considered equal, even if they have fields that differ
    Given An Entity with name 'note'
