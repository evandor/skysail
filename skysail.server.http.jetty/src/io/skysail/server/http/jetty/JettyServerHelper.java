...
package io.skysail.server.http.jetty;
...
import io.skysail.server.http.jetty.internal.OSGiServerConstants;
import io.skysail.server.http.jetty.internal.ServerDecorator;
import io.skysail.server.http.jetty.internal.ServerInstanceWrapper;
import io.skysail.server.http.jetty.internal.Util;
...
public abstract class JettyServerHelper extends org.restlet.engine.adapter.HttpServerHelper {
    ...
    private org.eclipse.jetty.server.Server createServer() {
        ...
        // Server
        // final org.eclipse.jetty.server.Server server = new
        // WrappedServer(this,
        // threadPool);

        org.eclipse.jetty.server.Server configuredJettyServer;
        try {

            Dictionary<String, String> properties = new Hashtable<>();
            String jettyHomeSysProp = "/Users/carsten/git/skysail/skysail.server.http/resources/jettyhome/";// Util.resolvePropertyValue(OSGiServerConstants.JETTY_HOME);
            // bug 329621
            ...
            File jettyHomeDir = new File(jettyHomeSysProp);
            ...
            List<URL> configURLs = /* jettyHomeDir != null ? */ getJettyConfigurationURLs(jettyHomeDir);// :
                                                                                                        // getJettyConfigurationURLs(jettyHomeBundle,
                                                                                                        // properties);
            ...
            configuredJettyServer = ServerInstanceWrapper.configure(null, configURLs, properties);

            Thread.currentThread().setContextClassLoader(ccl);


            org.eclipse.jetty.server.Server server = configuredJettyServer;
            // Connector
            ...
            return new ServerDecorator(server,this);
        ...
        // The JettyServerServiceTracker will notice it and set it up to deploy
        // bundles as wars etc
        // bundleContext.registerService(Server.class.getName(), server,
        // properties);
    ...
    private static List<URL> getJettyConfigurationURLs(File jettyhome)
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            // etc files can either be relative to jetty.home or absolute disk
            // locations
