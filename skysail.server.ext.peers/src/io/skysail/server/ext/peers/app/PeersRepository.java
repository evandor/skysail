...
public class PeersRepository extends GraphDbRepository<PublicPeerDescription> implements io.skysail.domain.core.repos.DbRepository {
    ...
    public PublicPeerDescription findByPeerIdentifier(String identifier) {
        dbService.register(PublicPeerDescription.class);
        String sql = "SELECT from "+DbClassName.of(PublicPeerDescription.class)+" WHERE peerIdentifier= :peerIdentifier";
        Map<String, Object> params = new HashMap<>();
        params.put("peerIdentifier", identifier);
        List<PublicPeerDescription> found = dbService.findObjects(sql,params);
       if (found.size() == 1) {
           return found.get(0);
       }
       return null;
