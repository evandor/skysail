...
class OAuth2CallbackResource extends EntityServerResource[GenericIdentifiable] {
  ...
  def getEntity(): GenericIdentifiable = {
    val storedState= getContext().getAttributes().get(FacebookApplication.FACEBOOK_AUTH_STATE).asInstanceOf[String];
...
//        }
//
        getContext().getAttributes().remove(FacebookApplication.FACEBOOK_AUTH_STATE);
