...
class OAuth2CallbackResource extends EntityServerResource[GenericIdentifiable] {
  ...
  def getEntity(): GenericIdentifiable = {
    val storedState = getContext().getAttributes().get(TemplateApplication.TEMPLATE_AUTH_STATE).asInstanceOf[String];
    ...
    //        }
    //
    getContext().getAttributes().remove(TemplateApplication.TEMPLATE_AUTH_STATE);
